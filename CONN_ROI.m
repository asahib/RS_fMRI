...
TARGETpath='/nafs/narr/asahib/MR_fix_MSMALL_GICA/CONN_analysis/outputs';                                                             % target folder for conn project (default current folder)
...
%     if OVERWRITE||isempty(dir(regexprep(f2,'\.gz$',''))), gunzip(f2); end; f2=regexprep(f2,'\.gz$','');
   ...
   STRUCTURAL_FILE{n,1}=t1;
    FUNCTIONAL_FILE(n,1:2)={f1,f2,f3,f4};
...
clear batch;
batch.filename=fullfile(TARGETpath,'conn_KET1_KET3.mat');
if RUNPARALLEL
