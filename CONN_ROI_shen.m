...
TARGETpath='/nafs/narr/asahib/MR_fix_MSMALL_GICA/CONN_analysis/output_shen';                                                             % target folder for conn project (default current folder)
...
for n=1:P
    ...
    f1=fullfile(sprintf(CONNECTOMEpath,subs{1,1}{n,1}),'Results','rest_acq-AP_run-01','drest_acq-AP_run-01_hp2000_clean.nii');   % FUNCTIONAL VOLUME (1/4)
    f2=fullfile(sprintf(CONNECTOMEpath,subs{1,1}{n,1}),'Results','rest_acq-PA_run-02','drest_acq-PA_run-02_hp2000_clean.nii');   % FUNCTIONAL VOLUME (2/4)
...
batch.Setup.rois.names={'shen_atlas'};   %% runs for shen atlas havimg 268 regions with 2mm resolution
batch.Setup.rois.files{1}=fullfile(fileparts(which('conn')),'rois','shen_atlas.nii');
